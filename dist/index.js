...
app.get("/projects/:projectId/tasks", async (req, res) => {
    const rows = await (0, tasks_repository_1.getTasks)();
    res.status(HTTP.OK).json(rows);
});
...
app.get("/projects/:id/with-tasks", async (req, res) => {
    ...
    const rows = await (0, project_with_tasks_repository_1.getProjectWithTasksJoin)(idNum);
    res.status(HTTP.OK).json(rows);
});
...
app.post("/projects/:projectId/tasks", async (req, res) => {
    ...
    const created = await (0, tasks_repository_1.createTask)({ title, project_id, is_done });
    res.status(HTTP.CREATED).json(created);
});
...
app.put("/tasks/:id", async (req, res) => {
    ...
    const updated = await (0, tasks_repository_1.updateTask)(idNum, {
        title: title.trim(),
        is_done
    });
    if (!updated) {
...
app.delete("/tasks/:id", async (req, res) => {
    ...
    const ok = await (0, tasks_repository_1.deleteTask)(idNum);
    if (!ok) {
        res.sendStatus(HTTP.NOT_FOUND);
        return;
    }
